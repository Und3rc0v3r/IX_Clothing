...
function populateCarousel(gList) {
	...
	gList.forEach(function (item) {
		...
		if (galleryCount == 0) {
			...
			rawhtml = `<div class="carousel-item">	<img class="d-block w-100" src="${item.url}" alt=" ${galleryCount}"> <div class="carousel-caption d-none d-md-block" style="background: rgba(0, 0, 0, 0.5); border-radius: 25px;	">	<h5>${itemsWornList}</h5>		<p>${item.description}</p>	 </div></div>`;
			addElement("carousel-inner", "div", "gallery-img-" + galleryCount, rawhtml);
